...
export async function generateResponse(review: string, author: string, businessName?: string, temperature?: number, emojiList?: string[]): Promise<string> {
  ...
  const emojiList_safe = emojiList?.length ? emojiList : ['😊', '🙌', '👏', '💪', '⭐', '❤️', '🔥', '🎯', '👍', '🌟', '🍽️', '🤝', '✅', '🙏', '✨', '🎉', '💎', '🤩', '😍', '💯', '🫶', '👀', '🏆', '🚗', '💇', '🦷', '🧹', '🌿', '🐾', '🎊'];
  const emojiRule = emojiList_safe.length
    ? `Puedes usar entre 0 y 3 emojis de esta lista, TÚ eliges cuáles según el contexto: ${emojiList_safe.join('')}. Colócalos DENTRO del texto, no al final. Preferiblemente en medio de una frase para enfatizar una palabra o idea. Si no encajan de forma natural en medio, mejor no ponerlos. Varía los emojis entre respuestas — no repitas el mismo emoji dos veces seguidas en distintas respuestas. Si el tono es serio, mejor sin emoji.`
